...
#define xRefCntr (fv->superBlock.iHeight - 3)
...
uint Inodes::create
  ...
  fv->superBlock.iDirect = iHeight - 1 - 1 -1 - iIndirect;	// see xType, xFileSize
...
uint Inodes::getLnCnt(uint in)
{
  return getEntry(in, xRefCntr);
}

uint Inodes::setLnCnt(uint in, uint tp)
{
  return setEntry(in, xRefCntr, tp);
}
...
void Inodes::softWrite(uint in, char* path)
{
  //Get a free block
  uint freeBlk = fv->getFreeBlock();
  
  //Write the path name into the retrieved block and set the entries of inode to block location and string length
  fv->writeBlock(freeBlk,path);
  setEntry(in , 0 , freeBlk);
  setEntry(in,7, strlen(path));
}

char* Inodes::softRead(uint in, char* str)
{
  //Retrieve stored path name in block at inode entry 0
  str = new char[256];
  fv->readBlock(getEntry(in,0),str);
  return str;
}
...
uint Inodes::show(uint in)
{
  ...
  //Update inode to show the link count, file type, and file size
  printf(" LinkCnt=%d", pin[x]);
  x++;
  printf(" Type=%d", pin[x]);
  x++;
  printf(" Size=%d]\n", pin[x]);
